...
export default function VehicleTable({ vehicles, canWrite }: Props) {
    ...
    const cols = ["Reg. No.", "Model", "Type", "Status", "Capacity", "Odometer", "Fuel", "Next Service"];
    if (canWrite) cols.push("");

    return (
        <>
            {/* Toolbar */}
            <div style={s.toolbar}>
                <span style={s.count}>{vehicles.length} vehicle{vehicles.length !== 1 ? "s" : ""} in fleet</span>
                {canWrite && (
                    <button style={s.addBtn} onClick={() => setShowAdd(true)}>+ Add Vehicle</button>
                )}
            </div>
            ...
            {vehicles.length === 0 ? (
                <div style={s.empty}>
                    No vehicles registered yet.{canWrite ? ' Click "+ Add Vehicle" to register one.' : ""}
                </div>
            ) : (
                        ...
                        <tbody>
                            {vehicles.map((v) => (
                                <tr key={v.id} style={s.tr}>
                                    ...
                                    <td style={{ ...s.td, color: "var(--muted-foreground)" }}>{v.model ?? "—"}</td>
                                    <td style={s.td}>{v.type.charAt(0).toUpperCase() + v.type.slice(1)}</td>
                                    <td style={s.td}><StatusBadge status={v.status} /></td>
                                    <td style={s.td}>{v.max_capacity ? `${v.max_capacity.toLocaleString()} kg` : "—"}</td>
                                    <td style={s.td}>{v.km_driven.toLocaleString()} km</td>
                                    <td style={s.td}><FuelBar percent={v.fuel_percent} /></td>
                                    <td style={s.td}>
                                    ...
                                    {canWrite && (
                                        <td style={{ ...s.td, ...s.actionsCell }}>
                                            ...
                                            <button style={s.deleteBtn} onClick={() => handleDelete(v.id)} disabled={deleting === v.id}>
...
function FuelBar({ percent }: { percent: number }) {
    const color = percent > 50 ? "oklch(0.7 0.17 150)" : percent > 20 ? "oklch(0.75 0.18 60)" : "var(--destructive)";
    return (
